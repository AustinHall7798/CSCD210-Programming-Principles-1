...
public class MyLinkedList {
	...
	public Object removeFirst() throws Exception {
		...
		ListNode returnedNode = this.head.next;
		this.head.next = returnedNode.next;
		size--;
		return returnedNode.data; //change this as you need.
	}
	...
	public boolean remove(Object o) {
		...
		for (cur = this.head.next; cur != null; prev = cur, cur = cur.next) {

			//first checks whether o is null so cur.data.equals(o) doesn't throw exception
			if ((o == null && cur.data == null) || (cur.data != null && cur.data.equals(o))) {
				if (prev == null) {
	...
	public boolean removeAllCopies( Object o ) { //passed test
		int count = 0;
		for(ListNode cur = this.head.next; cur != null; cur = cur.next) {
			// Uses remove method for each node. If something is removed, count is incremented
			// since remove returns true. If count is greater than when initialized, we know 
	...
	public void add(int index, Object o) throws IndexOutOfBoundsException{
		...
		ListNode prev = null;
		int i = 0;
		for(cur = this.head.next; i < index; i++, prev = cur, cur = cur.next);
